	...
	printed_version = TRUE;
      ...
      /* Mirror left-right or top-bottom. */
			...
			if (keymatch(argv[argn], "horizontal", 1)) {
				if (select_transform(JXFORM_FLIP_H) != 0)
					return -1;
			}
			else if (keymatch(argv[argn], "vertical", 1)) {
				if (select_transform(JXFORM_FLIP_V) != 0)
					return -1;
			}
				else {
					usage();
	...
	/* restart_interval will be computed during startup */
      ...
      /* Rotate 90, 180, or 270 degrees (measured clockwise). */
			...
			if (keymatch(argv[argn], "90", 2)) {
				if (select_transform(JXFORM_ROT_90) != 0)
					return -1;
			}
			else if (keymatch(argv[argn], "180", 3)) {
				if (select_transform(JXFORM_ROT_180) != 0)
					return -1;
			}
			else if (keymatch(argv[argn], "270", 3)) {
				if (select_transform(JXFORM_ROT_270) != 0)
					return -1;
			}
			else {
				usage();
...
int main(int argc, const char** argv)
{
  ...
  struct jpeg_error_mgr jsrcerr = { 0 };
#if TRANSFORMS_SUPPORTED
  ...
  struct jpeg_error_mgr jdsterr = { 0 };
